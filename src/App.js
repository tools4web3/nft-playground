...
function App() {
  ...
  return (
    <div className="App">
      {
        !isLoggedIn ? <button onClick={() => connect()}>Connect</button> : <h2>{`logged in as ${accounts[0]}`}</h2>
      }
      
      <h1>Signer</h1>
      <form>
        <h2>Domain</h2>
        <input type="text" placeholder="name (string)" value={name} onChange={e => setname(e.target.value)} />
        <input type="text" placeholder="version (string)" value={version} onChange={e => setversion(e.target.value)} />
        <input type="text" placeholder="chainId (must be the same as current net)" value={chainId} onChange={e => setchainId(e.target.value)} />
        <input type="text" placeholder="verifyingContract (address, like 0xCcCCccccCCCCcCCCCCCcCcCccCcCCCcCcccccccC)" value={verifyingContract} onChange={e => setverifyingContract(e.target.value)} />

        <h2>Content</h2>
        <input type="text" placeholder="Signer (address, like 0xCcCCccccCCCCcCCCCCCcCcCccCcCCCcCcccccccC)" value={Signer} onChange={e => setSigner(e.target.value)} />
        <input type="text" placeholder="IsListing (bool)" value={IsListing} onChange={e => setIsListing(e.target.value)} />
